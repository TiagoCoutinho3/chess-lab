...
import {
  pickIdentityDialogue,
  BIG_EVAL_SWING_CP,
} from "../data/botIdentityDialogue";
...
export const PlayView: React.FC<PlayViewProps> = ({ initialBot }) => {
  ...
  const [isAnalysisEnabled, setIsAnalysisEnabled] = useState<boolean>(true);
  ...
  const setTemporaryMood = useCallback(
    (mood: BotAvatarMood, durationMs: number) => {
  ...
  useEffect(() => {
    if (speechVisible) {
      setBotMood((current) =>
        current === "idle" || current === "speaking" ? "speaking" : current,
      );
    }
  }, [speechVisible]);
  ...
  const getCapturedPieces = () => {
    ...
    chess.board().forEach((row) => {
      row.forEach((piece) => {
        ...
        if (piece.color === "w") {
          currentCountsWhite[piece.type] =
            (currentCountsWhite[piece.type] || 0) + 1;
        } else {
          currentCountsBlack[piece.type] =
            (currentCountsBlack[piece.type] || 0) + 1;
        }
      });
    });

    (Object.keys(initialCounts) as (keyof typeof initialCounts)[]).forEach(
      (type) => {
  ...
  const resetGame = (botToUse?: Bot, colorToUse?: "w" | "b") => {
    ...
    if (
      colorToUse === "b" ||
      (colorToUse === undefined && playerColor === "b")
    ) {
  ...
  const handleGameOver = useCallback(
    (
      ...
      const playedGame: PlayedGame = {
        ...
        pgn: chess.pgn(),
        moves: gameMoves,
        playerAccuracy: Math.min(
          98,
          Math.max(65, Math.floor(85 + Math.random() * 10)),
        ),
      };
    ...
    (currentChessInstance: Chess) => {
      if (currentChessInstance.isGameOver()) {
        if (currentChessInstance.isCheckmate()) {
          ...
          let reason = "Empate";
          if (currentChessInstance.isStalemate())
            reason = "Empate por afogamento";
          else if (currentChessInstance.isThreefoldRepetition())
            reason = "Empate por repetição de lances";
          else if (currentChessInstance.isInsufficientMaterial())
            reason = "Empate por material insuficiente";
  ...
  useEffect(() => {
    if (!currentOpening || announcedOpeningRef.current === currentOpening.eco)
      return;
  ...
  const triggerBotMove = useCallback(
    async (chessInstance: Chess) => {
      ...
      if (chessInstance.isGameOver() || chessInstance.turn() !== botTurn)
        return;
        ...
        if (result) {
          ...
          if (result.captured) {
            ...
            const line = pickTraitDialogue(
              currentBot.traits,
              "capture",
              result.san.includes("x") && currentBot.traits.includes("artista")
                ? "artista"
                : undefined,
            );
            showBotSpeech(line);
          ...
          if (prevSwing !== null) {
            if (
              botEvalCp >= BIG_EVAL_SWING_CP &&
              prevSwing < BIG_EVAL_SWING_CP
            ) {
              showBotSpeech(
                pickIdentityDialogue(currentBot.id, "bigAdvantage"),
              );
              ...
              showBotSpeech(
                pickIdentityDialogue(currentBot.id, "bigDisadvantage"),
              );
  ...
  const handlePlayerMove = async (moveData: {
    ...
    analyzeMove(fenBefore, moveResult.san)
      .then((analysis) => {
        ...
        const isBadMove = ["mistake", "blunder", "inaccuracy"].includes(
          analysis.quality,
        );
  ...
  const handleRequestHint = async () => {
    ...
    sounds.playHint();
    const { bestMove, score, evaluationFormatted } = await findBestMove(
      chess,
      4,
    );
  ...
  const handleUndo = () => {
    ...
    const movesToKeep =
      chess.turn() === "b" ? gameMoves.slice(0, -1) : gameMoves.slice(0, -2);
  ...
  const lastPlayerAnalysis =
    moveAnalyses.length > 0 ? moveAnalyses[moveAnalyses.length - 1] : null;

  const isPlayerTurnActive =
    chess.turn() === playerColor && !gameOverResult?.isOver;
  const currentEvaluationCp =
    moveAnalyses.length > 0
      ? moveAnalyses[moveAnalyses.length - 1].evaluationAfter
      : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4 items-start">
        ...
        <div className="flex flex-col gap-2 sm:gap-3">
          ...
          >
            <div className="p-3 sm:p-4 flex gap-3 sm:gap-4 items-start">
              <div className="relative shrink-0">
                <div
                  className={`w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden border bg-[#F7F9FC] transition-all duration-300 ${botMoodRingClass}`}
                >
                  <BotAvatar
            ...
            <div className="flex flex-wrap gap-2 px-3 pb-3 sm:px-4 sm:pb-4">
              ...
              <button
                id="change-color-btn"
                onClick={() =>
                  resetGame(currentBot, playerColor === "w" ? "b" : "w")
                }
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#F7F9FC] hover:bg-[#EDE7FF] border border-[#DDE3EA] hover:border-[#8B5CF6] text-[11px] font-bold text-slate-700 transition-all"
              >
          ...
          <div className="flex-1 min-h-0 max-h-[38vh] bg-white rounded-2xl p-3 sm:p-4 border border-[#DDE3EA] shadow-xs flex flex-col">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 shrink-0 gap-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    aria-pressed={isAnalysisEnabled}
                    ...
                    className={`relative inline-flex h-6 w-12 items-center rounded-full border transition-colors ${
                      isAnalysisEnabled
                        ? "bg-[#8AA7E1] border-[#8AA7E1]"
                        : "bg-slate-200 border-slate-300"
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${
                        isAnalysisEnabled ? "translate-x-7" : "translate-x-1"
                      }`}
                    />
                  </button>
                </div>
            ...
            {isAnalysisEnabled && (
              <div className="pt-3 pb-2 shrink-0">
                <EvaluationBar evaluationCp={currentEvaluationCp} />
              </div>
            )}
                  ...
                  {Array.from({ length: Math.ceil(gameMoves.length / 2) }).map(
                    (_, idx) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-slate-400 w-7 font-semibold">
                            {idx + 1}.
                          </span>
                          <div className="flex-1 flex items-center gap-1.5 font-bold text-slate-800">
                            <span>{whiteMove}</span>
                            {isAnalysisEnabled && whiteAnalysis && (
                              <MoveEvaluationBadge
                                quality={whiteAnalysis.quality}
                                showLabel={false}
                              />
                            )}
                          </div>
                          <div className="flex-1 flex items-center gap-1.5 font-medium text-slate-600">
                            <span>{blackMove || ""}</span>
                            {isAnalysisEnabled && blackAnalysis && (
                              <MoveEvaluationBadge
                                quality={blackAnalysis.quality}
                                showLabel={false}
                              />
                            )}
                          </div>
          ...
          >
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden border border-[#DDE3EA] bg-[#8AA7E1]/10 shrink-0">
              <PlayerAvatar alt="Você" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="font-black text-base sm:text-lg text-slate-800">
                Você ({playerColor === "w" ? "Brancas" : "Pretas"})
              </h4>
              {isAnalysisEnabled && lastPlayerAnalysis && (
                <div className="mt-1">
                  <MoveEvaluationBadge
                    quality={lastPlayerAnalysis.quality}
                    showLabel={false}
                  />
                </div>
              )}
